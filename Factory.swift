...
class Factory {
    ...
    class func createSlot(currentCards:[Slot]) -> Slot {
        ...
        let cMaxSlotValue = 13
        
        var randomNumber = Int(arc4random_uniform(UInt32(cMaxSlotValue)))
        while contains( currentCardValues, randomNumber+1 ){
            randomNumber = Int(arc4random_uniform(UInt32(cMaxSlotValue)))
        }
